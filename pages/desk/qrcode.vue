<template>
	<view>
		<view class='qrcode-box'>
			...
			<view class="txt-center color-666">{{txt}}</view>
			<view class='qrcode-opretion'>
				<button @click='savePic' class='qrcode-btn'>保存二维码</button>
			</view>
		</view>
...
<script>
	...
	let QRCode = require('../../util/qrcode.js').default
	...
	export default {
		...
		data() {
			return {
				...
				QR: '', // 二维码
				qrText: '', // 输入的二维码内容
				logoImg: '../../static/images/mine/bg.jpg', // logo图片
				color: '#000000', // 选择的颜色，默认：黑色
				colorData: {r: 0,g: 0,b: 0,a: 1}
			}
		},
		onLoad ({text}) {
			this.qrText = this.qrCodeUrl
			this.txt = text
			let that = this
